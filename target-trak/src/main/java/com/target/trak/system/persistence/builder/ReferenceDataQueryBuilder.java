...
public class ReferenceDataQueryBuilder {
	...
	public String buildSearchCriteriaReferenceDataPaginatedQuery(final ReferenceDataSearchCriteria criteria, final MapSqlParameterSource params) {
		...
		if (!StringUtils.isEmpty(criteria.getCreatedBy())) {
			builder.append(QueryConstantsEnum.AND.value).append(CREATED_BY_COLUMN).append(QueryConstantsEnum.EQUALS.value).append(":createdBy");
			params.addValue("createdBy", criteria.getCreatedBy());
		}

		if (!StringUtils.isEmpty(criteria.getLastUpdatedBy())) {
			builder.append(QueryConstantsEnum.AND.value).append(LAST_UPDATED_BY_COLUMN).append(QueryConstantsEnum.EQUALS.value).append(":lastUpdatedBy");
			params.addValue("lastUpdatedBy", criteria.getLastUpdatedBy());
		}
